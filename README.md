<!-- =========================
      KABIR PURI — GITHUB PROFILE
      COMPLETE README (ALL SECTIONS)
========================= -->

<!-- 🔥 Banner (Upload your own banner.png then replace link) -->
<p align="center">
  <img src="https://raw.githubusercontent.com/Kabixr/Kabixr/main/banner.png" width="100%" alt="Kabir Banner"/>
</p>

<h1 align="center">✨ Kabir Puri ✨</h1>
<h3 align="center">Data Analytics • AI/ML Engineering • Intelligent Systems Developer</h3>

<p align="center">
  <img src="https://img.shields.io/badge/Data%20Analytics-Expert-blue?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/AI%2FML-Engineer-brightgreen?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/LLM%20Developer-yellow?style=for-the-badge"/>
</p>


---

# 🌟 About Me  

I build **AI-driven, data-powered systems** that deliver measurable impact.  
From healthcare to finance, I focus on creating solutions that are **beyond theoretical**—  
systems that genuinely improve workflows, accuracy, speed, and user experience.

My philosophy:  
**Understand deeply → Build intelligently → Automate thoughtfully → Deliver impact.**

---

# 🏆 Professional Experience (Subtle Flex, Just Facts)

### 🏥 **Medanta – The Medicity | Customer Success & Analytics Intern**  
_mar 2025 – oct 2025_  
- Increased digital adoption by **~20%**  
- Eliminated **~95% duplicate entries** using automated validation  
- Improved reporting efficiency by **~20%**  
- Reduced manual workflow errors by **~30%**

---

### 💹 **J.P. Morgan Chase & Co. | Software Engineering Intern (2024)**  
- Developed real-time stock dashboard (**1000+ updates/sec**)  
- Improved trader decision-making speed **~20%**  
- Built **10+ reusable frameworks** improving engineering onboarding  
- Automated validation pipelines for financial systems  

---

### 🖥️ **J.P. Morgan Chase & Co. | Software Engineering Intern (2021)**  
- Engineered enterprise database (10k+ records) → **40% faster retrieval**  
- Optimized SQL/JDBC queries **5s → <1s**  
- Reduced IT support tickets **~30%** via training & workflow upgrades  

---

# 🎓 Education  

### **MCA — Artificial Intelligence & Machine Learning**  
Amity University  
- Research: SQL Optimization using LLMs → **25% faster queries**  
- Project: Weapon Detection (YOLOv8) → **94% accuracy**

### **BCA — Information Technology**  
Amity University  

---

# 💡 Skills & Technologies  

### **Languages**  
`Python` · `Java` · `SQL`

### **AI / ML**  
`Scikit-learn` · `LLMs` · `LangChain` · `Predictive Analytics` · `Computer Vision`

### **Data Engineering & Analytics**  
`Pandas` · `NumPy` · `Power BI` · `Tableau`

### **Software Engineering**  
`React.js` · `REST APIs` · `Automation Pipelines`

### **Soft Skills**  
System Thinking · Leadership · Communication · Problem Solving  

---

# 🎨 Badge Wall (Tech Flex)
<p align="center">

<img src="https://img.shields.io/badge/Python-3776AB.svg?style=for-the-badge&logo=python&logoColor=white"/>
<img src="https://img.shields.io/badge/Java-007396.svg?style=for-the-badge&logo=java&logoColor=white"/>
<img src="https://img.shields.io/badge/SQL-025E8C.svg?style=for-the-badge&logo=postgresql&logoColor=white"/>

<br/>

<img src="https://img.shields.io/badge/Scikit--Learn-F7931E.svg?style=for-the-badge&logo=scikit-learn&logoColor=white"/>
<img src="https://img.shields.io/badge/LangChain-000000.svg?style=for-the-badge"/>
<img src="https://img.shields.io/badge/LLMs-000000.svg?style=for-the-badge&logo=openai"/>

<br/>

<img src="https://img.shields.io/badge/Pandas-150458.svg?style=for-the-badge&logo=pandas&logoColor=white"/>
<img src="https://img.shields.io/badge/Numpy-013243.svg?style=for-the-badge&logo=numpy&logoColor=white"/>
<img src="https://img.shields.io/badge/PowerBI-F2C811.svg?style=for-the-badge&logo=powerbi&logoColor=black"/>
<img src="https://img.shields.io/badge/Tableau-E97627.svg?style=for-the-badge&logo=tableau&logoColor=white"/>

</p>

---

# 🧩 Featured Projects  

### 🔹 **Real-Time Weapon Detection (YOLOv8)**  
94% accuracy, real-time inference on video streams  

### 🔹 **SQL Optimization Assistant (LLM-Powered)**  
Automates SQL tuning, validation & efficiency improvements  

### 🔹 **Emotion-Aware Music Recommender**  
Maps mood → playlist using NLP + ML  

### 🔹 **Analytics Case Studies**  
Uber Analytics · Sneaker Market · Food Delivery · iPhone Sales · Spotify  

---

**Kabir Puri — Data Analytics & AI/ML Engineer**  
_Engineering clarity. Automating intelligence. Building systems that last._

---

# 🎯 My Hobbies & Interests  
Photography · Fitness · Music Production · Data Experiments · Creative Tech Projects  

---

# 📄 Resume  
**[Download My Resume](https://raw.githubusercontent.com/Kabixr/Kabixr/main/resume.pdf)**  
(Upload your resume as `resume.pdf` in this repo)

---

# 📊 GitHub Stats  

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=Kabixr&show_icons=true&theme=tokyonight" width="50%"/>
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=Kabixr&theme=tokyonight" width="50%"/>
</p>

---

# 🔗 Connect With Me  
**Email:** kabixr.puri@gmail.com  
**LinkedIn:** https://linkedin.com/in/kabixr  
**GitHub:** https://github.com/Kabixr  

---

<p align="center">
  <b>Thank you for visiting — let’s build something extraordinary.</b>
</p>
